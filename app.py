...
import datetime
from database import accept_a_request, add_request, create_borrowing_history_table, create_users_table, extend_date, find_book, get_issued_books, get_requests, get_user_requests, get_user_requestss, ifcanextend, register_user, authenticate_user, reject_a_request
...
def add_book_page():
    ...
    genre = st.text_input("Genre or Department and Department :")
...
def view_books_page():
    ...
    if not books:
        ...
        st.table({
            ...
            'Genre or Department': [book[3] for book in books],
...
def search_books_page():
    ...
    search_category = st.selectbox("Select search category:", [
                                   "Book Name", "Author", "Genre or Department", "ISBN"])
    ...
    if st.button("Search"):
        ...
        if not books:
            ...
            for i, book in enumerate(books):
                ...
                st.write(f"**Genre or Department:** {book[3]}")
...
def search_books(category, query):
    ...
    elif category == "Genre or Department":
...
def borrowing_history_page():
    ...
    if st.session_state.user_id:
        ...
        returned_book = ifcanextend(user_id)
        ret_books = [book[2] for book in returned_book]
